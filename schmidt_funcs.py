...
def mavg(arr, n = 2):
    ...
    if len(arr) > 400:
        ...
        return signal.convolve(arr, [1./float(n)] * n, mode = 'valid')
...
def pdf(extmap,bins):
    ...
    '''
    extmap = comp(extmap)
    pdf,x = np.histogram(extmap,bins=bins,density = False)
    pdf = pdf/(np.sum(pdf) * np.diff(bins))
    return pdf

def pdf2(extmap,bins):
    '''
...
def hist(extmap,bins):
    ...
    return hist.astype(np.float)


def bootstrap(ext,err):
    return ext + err * np.random.randn()

def num_above(extmap,level):
    extmap = comp(extmap)
...
def schmidt_results_plots(sampler, model, x, y, yerr, burnin = 200, akmap = None, bins = None, scale = None, triangle_plot = True):
    ...
    if (akmap is not None) & (bins is not None):
        plt.plot(bins[:-1],1./(diff_area_function(akmap,bins)*scale),'o',mec='none',alpha=0.75)

    plt.errorbar(x,y,yerr,fmt='rs',alpha=0.7,mec = 'none')
    plt.legend(['Inv. Diff, Area function','Data'],loc='upper left', fontsize = 12)
...
def fit(bins, samp, samperr, maps, mapserr,
        scale = 1., pos = None, pose = None,
        title = None, sampler = None, no_plot = False, triangle_plot = True,
        nwalkers = 100, nsteps = 1e4):
    ...
    nyerr =[]
    for i in np.ma.MaskedArray(yp,mask = (~np.isfinite(yp) & ~np.isfinite(1/yp))).T:
